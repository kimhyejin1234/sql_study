...
--서식문자와 함께 사용하고 싶은 문자를 "" 로 묵어 전달합니다.
SELECT
    first_name, TO_CHAR(hire_date,'YYYY"년" MM"월" DD"일"')
FROM employees;

--숫자를 문자로 TO_CHAR(값,형식)
--형식에서 사용하는 '9' 는 질제 숫자 9가 아니라, 자릿수를 표현하기 위한 기호입니다.
SELECT TO_CHAR(20000,'99999') FROM dual;
--주어진 자릿수에 숫자를 모두 표기할 수 없어서 모두 #으로 표기합니다.
...
--주어진 문자열을 모두 변환해야 한다. 아래는 오류
SELECT TO_date('2020-03-31 12:23:50','YY-MM-DD') FROM dual;


-- XXXX년 XX월 XX 일 문자열 형식으로 변환해 보세요.
...
--NULL 변환함수 NVL2(컬럼,null 이 아닐 경우의 갑,null 일 경우의 값)
SELECT 
    NVL2('abc','널아님','널임')
FROM employees;    
...
SELECT
    DECODE('C','A','A입니다.','B','B입니다.','C','C입니다.','뭐데 그게?')
FROM dual;
