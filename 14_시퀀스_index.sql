...
CREATE SEQUENCE dept2_seq
    ...
    NOCYCLE; -- 순환 여부(기본값 : NOCYCLE, 순환시키려면 CYCLE)
...
/*
테이블 조회 시 인텍스가 붙은 컬럼을 조건절로 사용한다면
테이블 전체 조회가 아닌, 컬럼에 붙은 인덱스를 이용해서 조회를 진행합니다.
인덱스를 생성하게 되면 지정한 컬럼에 rowid 를 붙인 인텍스가 준비되고,
조회를 진행할 시 해당 인덱스의 rowid 를 통해 빠르 스캔을 가능하게 합니다.
*/
DROP INDEX emp_salary_idx;


-- 시퀀스와 인텍스를 사용하는 hint 방법
