...
트리거 자체를 트랜젝션의 일부로 처리하기 때문에 COMMIT 나 ROLLBACK  을 포함 할 수 없다.
*/
...
CREATE TABLE tb1_user_backup(
    ...
    name VARCHAR2(20),
    address VARCHAR2(30),
    update_date DATE DEFAULT sysdate,--정보 변경시간(기본값:inser 되는 시간)
...
DECLARE
    v_type VARCHAR2(10);
BEGIN
    --현재 트리거가 발동된 상황이 update 인지 delete 인지 파악하는 조건문
    IF UPDATING THEN -- UPDATING 은 시스템 자체에서 상태에 대한 내용을 지원하는 빌트인 구문
        v_type := '수정';
    ELSIF DELETING THEN
        v_type := '삭제';
    END IF;
    
    --본격적인 실행 구문 작성(backup table 에 값을 insert 
    -- -> 원본 테이블에서 update or delete 된 사용자 정보 및 기타 정보)
    INSERT INTO tb1_user_backup
...
INSERT INTO tbl_user VALUES('test03','park','부산');
...
INSERT INTO tbl_user VALUES('test05','메뚜기','광주');
SELECT * FROM tbl_user;
SELECT * FROM tb1_user_backup;
ROLLBACK;
--------------------------------------------------------
...
BEGIN
    ...
    EXCEPTION 
        WHEN quantity_shortage_exception THEN
            ...
            --첫번째 매개값:에러코드(사용자 정의 예외 -20000~-20999까지)
            --두번째 매개값:에러 메세지
            RAISE_APPLICATION_ERROR(-20001,'주문 수량보다 재고가 적어서 주문할 수 없습니다.');
...
--트리거 내에서 예외가 발생하면 수행 중인 INSERT 작업은 중단되며 ROLLBACK 이 진행됩니다.
INSERT INTO order_history VALUES(order_history_seq.NEXTVAL,201,3,5,30000);