/*
# 서브쿼리 
: SQL 문장 안에 또다른 SQL 을 포함하는 방식.
여러개의 질의를 동시에 처리할 수 있습니다.ㅣ
WHERE,SEKECT,FROM 절에 작성이 가능합니다.
...
-- employee_id 가 103 번인 사람의 job_id 와 동일한 job_id 를 가진 사람을 조회.
SELECT 
    *
FROM employees
...
--단일 행 연산자: 주로 비교 연산자(=,>,<,>=,<=,<>) 을 사용하는 경우 하나의 행만 반환해야 합니다.
...
-- first_name 이 David 인 사람들의 금여와 같은 급여를 받는 사람들을 조회
...
--ALL:값을 서브퀴레이 의해 리턴된 각각의 값과 모두 비교해서
--모두 만족해야 합니다.
SELECT * FROM  employees
WHERE salary > ALL (SELECT salary FROM  employees
                WHERE first_name = 'David') ;

--EXISTS:서브퀴리가 하나 이상의 행을 반환하면 참으로 간주.
-- job_history 에 존재하는 직원이 employees 에도 존재한다면 조회에 포함
...
--특정 테이블 전체가 아닌 SELECT 를 통해 일부 데이터를 조회한 것을 가상 테이불로 사용하고 싶을 때.)
...
WHERE rn > 0 and rn <= 10; -- where 절의 rn 에 오류 발생(nvalid identifier)
