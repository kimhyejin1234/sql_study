...
FROM info
JOIN auth --JOIN 이라고만 쓰면 기본 INNER JOIN
...
--좌측 에티블과 우측 테이블 데이터를 모두 읽어 중복된 데이테는 삭제되는 외부 조인
SELECT
    *
FROM info i FULL JOIN auth a
on i.auth_id = a.auth_id;

--cross join 은 join 조건을 석정하지 않기 때문에
--단순히 모든 컬럼에 대해 join 을 진행합니다.
--실제로는 거의 사용하지 않습니다.
SELECT
    *
FROM info i 
